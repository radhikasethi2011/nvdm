    ...
    def __init__(
        ...
        with tf.variable_scope("encoder"):
            ...
            self.kld = -0.5 * tf.reduce_sum(
                1 - tf.square(self.mean) + 2 * self.logsigm - tf.exp(2 * self.logsigm),
                1,
            )
        ...
        for switch in range(0, 2):
            ...
            for i in range(alternate_epochs):
                ...
                for idx_batch in train_batches:
                    ...
                    _, (loss, kld, mean) = sess.run(
                        (optim, [model.objective, model.kld, model.mean]), input_feed
                    )
                    print(
                        "=================================================================== printing sess.run from train"
                    )
                    # print(sess.run(mean))
                ...
                print("length of mean outside for", len(mean))
                print("shape of mean outside for", mean.shape)
